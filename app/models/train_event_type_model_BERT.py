...
# Mappage des events aux indices
EVENT_TYPES_LABELS = {label: idx for idx, label in enumerate(config['CLASSIFIERS']['event_types'].split(','))}
...
default_event_type = EVENT_TYPES_LABELS.get("Apps Events")
event_type_labels = [EVENT_TYPES_LABELS.get(item['event_type'], default_event_type) for item in data]
...
# Nombre d'events
num_event_types = len(EVENT_TYPES_LABELS)
print("Total number of event_types:", num_event_types)

# Initialiser le modèle DistilBert pour la classification de séquences
model = DistilBertForSequenceClassification.from_pretrained('distilbert-base-uncased', num_labels=num_event_types)
